...
func (b *BillingCenter) CalculatePrice(c *eventsourced.Context, items *CalculatePrice) (*TotalPrice, error) {
	var totolPrice int32 = 0
	for _, v := range items.GetProducts() {
		...
		totolPrice = totolPrice + price*v.GetQuantity()
	}
	return &TotalPrice{
